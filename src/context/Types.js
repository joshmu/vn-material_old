// PLAYER
export const SET_PLAYER = 'SET_PLAYER'
export const SET_URL = 'SET_URL'
export const TOGGLE_PLAY = 'TOGGLE_PLAY'
export const ON_READY = 'ON_READY'
export const ON_START = 'ON_START'
export const ON_PLAY = 'ON_PLAY'
export const ON_PAUSE = 'ON_PAUSE'
export const ON_DURATION = 'ON_DURATION'
export const ON_PROGRESS = 'ON_PROGRESS'
export const LOAD_VIDEO = 'LOAD_VIDEO'

// NOTES
export const TOGGLE_OPEN = 'TOGGLE_OPEN'
export const ADD_TODO = 'ADD_TODO'
export const UPDATE_TODO = 'UPDATE_TODO'
export const REMOVE_TODO = 'REMOVE_TODO'
export const UPDATE_NEW_TODO = 'UPDATE_NEW_TODO'
export const SET_ACTION_INPUT_REF = 'SET_ACTION_INPUT_REF'
export const SET_TODOS = 'SET_TODOS'

// GLOBAL
export const TOGGLE_SETTINGS_OPEN = 'TOGGLE_SETTINGS_OPEN'
export const TOGGLE_ABOUT_OPEN = 'TOGGLE_ABOUT_OPEN'
export const TOGGLE_NOTES_DRAWER_OPEN = 'TOGGLE_NOTES_DRAWER_OPEN'
export const SET_BUFFER_SECONDS = 'SET_BUFFER_SECONDS'
